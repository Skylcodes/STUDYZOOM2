...
export async function addExampleData(
  ...
  for (let i = 0; i < contacts.length; i += BATCH_SIZE) {
    ...
    for (const contact of batch) {
      ...
      batchQueries.push(
        (prisma as any).studySet.create({
          data: {
            id: contactId,
            studyGroupId: studyGroupId,
            title: contact.name,
            filePath: imageUrl,
            recordType: contact.record,
            email: contact.email,
            phone: contact.phone,
            ...
            activities: {
              create: {
                ...
                metadata: detectChanges(null, {
                  ...
                  tags: []
                } as any), // Using type assertion to avoid TypeScript errors during refactoring
...
const contacts = [
  // COMPANIES
  {
    ...
    record: StudySetRecordType.COMPANY,
  ...
  // PEOPLE
  {
    ...
    record: StudySetRecordType.PERSON,
